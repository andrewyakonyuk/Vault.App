...
(function(document) {
    ...
    // the hide url bar
    boilerplate.BODY_SCROLL_TOP = false;
    ...
    boilerplate.fastButton.prototype.onClick = function(event) {
        ...
        if (event.type == 'touchend') {
            boilerplate.preventGhostClick(this.startX, this.startY);
        }
    ...
    boilerplate.ghostClickHandler = function(event) {
        if (!boilerplate.hadTouchEvent && boilerplate.dodgyAndroid) {
            ...
            // We pick that event up here, but boilerplate.coords is empty, because it's a new page,
        ...
        for (var i = 0, len = boilerplate.coords.length; i < len; i += 2) {
            var x = boilerplate.coords[i];
            var y = boilerplate.coords[i + 1];
    ...
    boilerplate.dodgyAndroid = ('ontouchstart' in window) && (navigator.userAgent.indexOf('Android 2.3') != -1);
    ...
    boilerplate.startupImage = function() {
        ...
        if ((navigator.platform === 'iPhone' || 'iPod') && (screen.height === 568)) {
            if (boilerplate.viewportmeta) {
                boilerplate.viewportmeta.content = boilerplate.viewportmeta.content
